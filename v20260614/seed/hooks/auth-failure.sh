...
# external tools (MCP servers, gh, git remote). When a tool fails because of
# auth/timeout (not a logic error), park the work in the deferred queue and tell
...
# Pull the error text from wherever it lives
...
# For MCP tools, also treat a bare "timeout/timed out" as auth-ish (the real
# overnight Hostinger case); for plain Bash, require a strong signal above.
MCP_RE='timed? ?out|timeout'
if printf '%s' "$err" | grep -Eiq "$AUTH_RE" \
   || { case "$tool" in mcp__*) printf '%s' "$err" | grep -Eiq "$MCP_RE";; *) false;; esac; }; then
  # Derive a service label
  svc="$tool"
  case "$tool" in
    mcp__hostinger__*) svc="hostinger-mcp" ;;  # remove if not using Hostinger MCP
  ...
  # Steer the agent: defer + continue, don't abort.
  python3 - "$svc" <<'PY'
import json,sys
svc=sys.argv[1]
