...
## Invocation
Triggers: ai slop, vibe coding remnants, leftover cruft, excess docs, dead code, tighten before shipping
...
1. **Default mode = report only.** Apply changes **only** if the user says "fix" / "apply". Even in fix mode: removals are surgical, one logical change per commit, and you never delete something whose purpose you can't explain.
2. **Read before judging.** Read the file and its callers before flagging — a "duplicate" may be intentional, a "dead" export may be a public API, a comment may encode a hard-won why. Misjudging is its own slop.

## IDENTIFYING SLOP

### 1. Excess & outdated documentation
...
Use `log--audit-results` → `template-findings.md`. Slug: the scoped target (e.g. `slop-songbook`).
...
## Related
- `ops--backlog` — follows: log slop findings as backlog items for systematic cleanup
- `audit--canary` — companion: canary for systemic bugs; slop for cruft removal
- `audit--drift` — companion: drift for goal alignment; slop for code cleanliness
- `audit--improve-codebase-architecture` — precedes: clean before deepening architecture
- `write--documentation` — companion: docs standards define what documentation is slop vs valuable
